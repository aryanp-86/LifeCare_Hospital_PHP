...
#welcomeuser{
  ...
  <div class="row">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      <li class="nav-item">
        <a href="/hms/dashboard_admin1.php" class="nav-link active text-white" aria-current="page">
...
<div class="container-xl">
  <h5 id="welcomeuser"><strong>Your Today's Appointments</strong>
  </h5>
</div>
<!-- </section> -->
<!-- </div> -->
...
<th scope='row'>".$row['sr.no']."</th>
      ...
      <td>" . $row['gender'] . "</td>
      <td>" . $row['slot'] . "</td>
      <td><button class='btn btn-success btn-sm' data-bs-toggle='modal' data-bs-target='#markModal'>Mark as done</button></td>
      </tr>";
}
if(isset($_POST['mark1'])){
    $ill=$_POST['ill'];
    $sno=1;
    $sql = "UPDATE `patientdata` SET  `illness` = '$ill' WHERE `sr.no` = '$sno';";
  $result = mysqli_query($conn,$sql);  
  $sno++;
  }
  ...
  if(isset($_POST['mark1'])){
    $sno=1;
    $sql = "INSERT INTO `patientdone` SELECT * FROM `patientdata`";
  $result = mysqli_query($conn,$sql);  
  $sno++;
  }
  if(isset($_POST['mark1'])){
  ...
  <strong>Success!</strong> Your appointment has been marked as complete!!
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h5 class="modal-title">Are you sure you want mark this appointment as complete?</h5>
</div>
<div class="modal-body">
<div class="col-12">
        <label for="inputill" class="form-label">Reason of illness</label>
        <input type="text" class="form-control" id="inputAddress" name="ill" required>
      </div>
      </div>
      <div class="modal-footer">
        <button type="submit" class="btn btn-success" name="mark1" >Mark as done</button>
